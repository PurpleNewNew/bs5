...
func (c *socks5Conn) Serve() (err error) {
	...
	if err == errAddressTypeNotSupported {
		_ = c.sendReply(request, socks5StatusAddressTypeNotSupported)
		return
	}
	if err != nil {
		return
	...
	default:
		_ = c.sendReply(request, socks5StatusCommandNotSupported)
...
func (c *socks5Conn) handleConnect(request *socks5Request) (err error) {
	_ = c.sendReply(request, socks5StatusSucceeded)
...
func (c *socks5Conn) handshake() (err error) {
	...
	if c.conf.Auth == nil {
		err = c.sendAuthReply(socks5AuthMethodNoRequired)
		return
	}
...
func (c *socks5Conn) authBasedPassword(methods []byte) (err error) {
	...
	if !bytes.Contains(methods, []byte{method}) {
		_ = c.sendAuthReply(socks5AuthMethodNoAcceptable)
		return errAuthMethodNotSupported
	}
	if err = c.sendAuthReply(method); err != nil {
		return
	}
	...
	if c.conf.Auth(string(username), string(password)) {
		_, err = c.localConn.Write([]byte{0x01, 0x01})
	} else {
		_, err = c.localConn.Write([]byte{0x01, 0x00})
	}
	return
}

func (c *socks5Conn) sendReply(request *socks5Request, status byte) error {
...
func (c *socks5Conn) sendReplyWithError(request *socks5Request, err error) bool {
	...
	case *net.OpError:
		switch err.Op {
		case "dial":
			_ = c.sendReply(request, socks5StatusHostUnreachable)
		case "read":
			_ = c.sendReply(request, socks5StatusConnectionRefused)
		}
	case syscall.Errno:
	...
	default:
		_ = c.sendReply(request, socks5StatusGeneral)
	}
	return true
}

func (c *socks5Conn) sendAuthReply(status byte) error {
	_, err := c.localConn.Write([]byte{socks5version, status})
	return err
}

func (c *socks5Conn) sendUDPReply(request *socks5Request) error {
	reply := []byte{0, 0}
	fragment := byte(0)
