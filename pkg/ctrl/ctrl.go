...
func Run(ctx context.Context, config *core.Suo5Config) error {
	...
	if config.OnRemoteConnected != nil {
		config.OnRemoteConnected(&core.ConnectedEvent{Mode: config.Mode})
	}
	...
	go func() {
		_ = srv.Serve(&core.ClientEventHandler{
			Inner:                   handler,
	...
	if !ok {
		...
		return fmt.Errorf("core can not work on this server")
	} else {
